...
public class UserServiceImpl implements IUserService {
	...
	public boolean addUser(User user,String auth){
		...
		UserCreateRequest createRequest = new UserCreateRequest(auth);
		...
		if(grouplist!=null&&grouplist.size()>0){
			if(password!=null&&!"".equals(password)&&alias!=null&&!"".equals(alias)){
				...
				JSONObject  object = (JSONObject) create(createRequest, auth);
...
//			}
	...
	public boolean deleteUser(User user,String auth){
		...
		if(user!=null&&user.getUserid()!=null&&!"".equals(user.getUserid())){
			...
			delete.getParams().add(user.getUserid());
			JSONObject object = (JSONObject)delete(delete, auth);
			if (object.has("result")) {
				result = true;
			}
		}
...
//		get.getParams().setSelectMediatypes("extend");
		...
		List<User> users = getUserBean(get, auth);
		return users;
	}
					...
					for(int i=0;i<array.length();i++){
						...
						if(userjson.has("medias")){
							...
							for (int j = 0; j < medias.length(); j++) {
								JSONObject object = (JSONObject) medias.get(j);
						...
						if(userjson.has("usrgrps")){
							...
							if(usrgrps!=null){
								for (int j = 0; j < usrgrps.length(); j++) {
									JSONObject object = (JSONObject) usrgrps.get(j);
...
//			}
	...
	public boolean updateUser(User user,String auth){
		...
		if(groups!=null&&groups.size()>0){
			...
			UserUpdateRequest updateRequest = new UserUpdateRequest(auth);
...
//					params.setMideas(medias);
					UserUpdatemediaRequest updatemedia = new UserUpdatemediaRequest(auth);
					UserAddmediaRequest addmedia =  new UserAddmediaRequest(auth);
					...
					JSONObject  object = (JSONObject) update(updateRequest, auth);
					if (object.has("result")) {
						result = true;
					} 
				}
