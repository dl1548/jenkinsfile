...
//	private Integer evaltype;		//required 计算方式		0 - AND / OR;1 - AND;2 - OR. 默认配置0
	...
	private List<ActionCondition> conditionsl;	//动作触发的条件
	private List<ActionOperation> operationsl;	//动作触发的操作
	private List<ActionFilter> filterl;//过滤
	
	
	public List<ActionCondition> getConditionsl() {
		return conditionsl;
	}
	public void setConditionsl(List<ActionCondition> conditionsl) {
		this.conditionsl = conditionsl;
	}
	public List<ActionOperation> getOperationsl() {
		return operationsl;
	}
	public void setOperationsl(List<ActionOperation> operationsl) {
		this.operationsl = operationsl;
	}
